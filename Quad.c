...
Quad *BST_search(Quad *root, int tx, int ty) {
   ...
   */
	...
	if(root == NULL)
		return NULL;
	int keet = tx + (ty * root->sx);
	Quad *found = NULL;
	if(root->key == keet){
		found = root;
	}
	else if(root->key > keet){
		found = BST_search(root->left, tx, ty);
	}
	else
		found = BST_search(root->right, tx, ty);
	...
	if(root != NULL)
	{
		...
		else
		{
			if(root->left == NULL){
				...
				Quad *smallest = find_successor(root->right);
				root->tx = smallest->tx;
				root->ty = smallest->ty;
				root->w = smallest->w;
				root->h = smallest->h;
				root->wsplit = smallest->wsplit;
...
int get_colour(Image *im, Quad *q) {
   ...
   */
	...
	int accumulator = 0;
	for(int i = q->tx; i < q->w + q->tx; i++){
		for(int j = q->ty; j < q->h + q->ty; j++)
			accumulator += im->data[i + (j * q->sx)];
	}
	
	return accumulator / (q->w * q->h);
}
   ...
   */
	...
	if(similar(im, root, threshold) == 0)
	{
		if(root->wsplit == 1)
		{
			...
			//Quad *new_left = new_Quad(root->tx, root->ty, new_w, root->h, 0, root->sx);
			root->w = new_w;
			root->wsplit = 0;
			
			//second child
			int new_tx = root->tx + new_w;
			Quad *second = new_Quad(new_tx, root->ty, new_w, root->h, 0, root->sx);
			
			//inserting into root
			BST_insert(root, second);
		}
		else
		{
			int new_h = root->h / 2;
			...
			//first child
			//Quad *new_left = new_Quad(root->tx, root->ty, root->w, new_h, 1, root->sx);
			
			root->h = new_h;
			root->wsplit = 1;
			
			//second child
			int new_ty = root->ty + new_h;
			Quad *second = new_Quad(root->tx, new_ty, root->w, new_h, 1, root->sx);
